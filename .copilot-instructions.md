...
git pull --rebase origin develop  # Pull again before push (CI/CD may have committed during our work)
git push origin develop
git fetch origin --tags --force  # Sync tags to avoid VS Code "overwrite" warnings
```

**Why pull twice?** CI/CD auto-bumps versions. It may bump between your first pull and your commit. The second `pull --rebase` catches any CI/CD commits and puts your work on top.
...
git merge develop --no-edit  # Pre-commit hook automatically allows version conflicts in merges
# If conflicts occur (usually VERSION.md, package.json, .csproj):
...
git fetch origin --tags --force  # Sync tags after merge
```

**Note:** The pre-commit hook automatically skips version file checks during merge commits (when `.git/MERGE_HEAD` exists), so no manual intervention is needed.
