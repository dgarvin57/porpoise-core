...
# All other version files are synced from this
...
CI_MODE=false
if [[ -n "$CI" ]] || [[ -n "$GITHUB_ACTIONS" ]] || [[ "$1" == "--ci" ]]; then
    CI_MODE=true
fi
...
if [[ "$CI_MODE" == true ]]; then
    echo -e "${BLUE}🤖 Running in CI mode (non-interactive)${NC}"
fi
echo

# Check if we're on a clean branch (skip in CI)
if [[ "$CI_MODE" == false ]] && [[ -n $(git status --porcelain) ]]; then
...
if echo "$COMMITS" | grep -q "BREAKING CHANGE:"; then
    ...
    if [[ "$CI_MODE" == true ]]; then
        echo -e "${BLUE}ℹ️  CI mode: No conventional commits, skipping version bump.${NC}"
        exit 0
    fi
