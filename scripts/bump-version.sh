...
# Uses conventional commit messages to determine version bump type
# Supports: feat: (minor), fix: (patch), BREAKING CHANGE: (major)
...
# Update PreferencesView.vue
echo "Updating porpoise-ui/src/views/PreferencesView.vue..."
sed -i.bak "s/const uiVersion = ref('$CURRENT_VERSION')/const uiVersion = ref('$NEW_VERSION')/" "$ROOT_DIR/porpoise-ui/src/views/PreferencesView.vue"
rm "$ROOT_DIR/porpoise-ui/src/views/PreferencesView.vue.bak"
...
git add "$ROOT_DIR/porpoise-ui/src/views/PreferencesView.vue"
